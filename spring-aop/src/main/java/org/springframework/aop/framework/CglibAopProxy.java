...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			/**
			 * 	整个函数完成的阐述了一个创建Spring中的Enhancer的过程.这里最重要的是通过getCallBacks方法设置连接器链
			 */
			// 设置拦截器
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		/**
		 * 		在getCallback中Spring考虑了很多情况,但是对于我们来说,只需要理解最常用的就可以了,比如将advised属性封装在DynamicAdvisedInterceptor
		 * 	并加入在callbacks中,这么做的目的是什么呢,如何调用呢?我们了解到CGLIB中对于方法的连接是通过将自定义的拦截器(实现MethodInterceptor接口)加
		 * 	入CallBack中并在调用代理时直接激活拦截器中的intercept方法来实现的,那么在getCallback中正是实现了这样一个目的,DynamicAdvisedInterceptor
		 * 	继承自MethodInterceptor,加入Callback中后,在再次调用代理时会直接调用DynamicAdvisedInterceptor中的intercept方法,由此推断,对于CGLIB方式
		 * 	实现的代理,其核心逻辑必然在DynamicAdvisedInterceptor中的intercept中.
		 * 		P200
		 */
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				/**
				 * 		上述的实现方式与JDK方式实现代理中的invoke方法大同小异,都是首先构造链,然后封装此链进行串联调用,
				 * 	稍有区别就是在JDK中直接构造ReflectiveMethodInvocation,而在cglib中使用CglibMethodInvocation. 
