...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		/**
		 * 		可以说事务是从这个函数开始的,因为在这个函数中已经开始尝试了对数据库连接的获取,当然,在获取数据库连接的同时,
		 * 	一些必要的设置也是需要同步设置的.
		 * 		1.尝试获取连接
		 * 				当然并不是每次都会获取新的连接,如果当前线程中的connectionHolder已经存在,则没有必要再次获取,或者,对于事务同步表示
		 * 			设置为true的需要重新获取连接.
		 * 		
		 * 		2.设置隔离级别以及只读标识
		 * 				你是否有过这样的错觉呢?事务中的只读配置是Spring中做了一些处理呢?Spring中确实是针对只读操作做了一些处理,但是核心
		 * 			的实现是设置connection上的readOnly属性.同样,对于隔离级别的控制也是交友connection去控制的.
		 * 		
		 * 		3.更改默认的提交设置
		 ...
		 * 		
		 * 		设置隔离级别的prepareConnectionForTransaction函数用于负责对底层数据库连接的设置,当然,只是包含只读标识和隔离级别的设置.
		 * 	由于强大的日志以及异常处理,显得函数代码量比较大,但是但从业务角度去看,关键代码其实是不多的.	
		 * 	
		 * 	P275
		 * 				
		 */
