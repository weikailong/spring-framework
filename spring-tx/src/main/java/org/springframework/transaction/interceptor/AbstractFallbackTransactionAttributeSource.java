...
public abstract class AbstractFallbackTransactionAttributeSource implements TransactionAttributeSource {
	...
	public TransactionAttribute getTransactionAttribute(Method method, @Nullable Class<?> targetClass) {
		...
		/**
		 * 		尝试从缓存中加载,如果对应信息没有被缓存的话,工作又委托给了computeTransactionAttribute函数.
		 */
	...
	protected TransactionAttribute computeTransactionAttribute(Method method, @Nullable Class<?> targetClass) {
		...
		/**
		 * 		对于事务属性的获取规则,如果方法中存在事务属性,则使用方法上的属性,否则使用方法所在的类上的属性,如果方法所在类的属性上
		 * 	还是没有搜寻到对应的事务属性,那么再搜寻接口中的方法,再没有的话,最后尝试搜寻接口的类上面的声明.对于函数computeTransactionAttribute
		 * 	中的逻辑与我们所认识的规则并无差别,但是上面函数中并没有真正的去做搜寻事务属性的逻辑,而是搭建了个执行框架,将搜索事务属性的
		 * 	任务委托给了findTransactionAttribute方法区执行.
		 * 		P267
		 */
