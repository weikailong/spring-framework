...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	// 用来表示一个bean的实例化倚靠另一个bean先实例化,对应bean属性depend-on
	@Nullable
	private String[] dependsOn;
	/**
	 * autowire-candidate属性设置为false,这样容器在查找自动装配对象时,将不考虑bean,即它不会被考虑作为其他bean自动装配的候选者,
	...
	// 定义这个bean的应用,APPLICATION:用户,INFRASTUCTURE:完全内部使用,与用户无关,SUPPORT:某些复杂配置的一部分, 程序设置
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		/**
		 * 		在Spring配置的中存在lookup-method和replace-method两个配置功能,而这两个配置的加载其实就是将配置统一存放在BeanDefinition
		 * 	中的methodOverrides属性里,这两个功能实现原理其实是在bean实例化的时候如果检测存在methodOverrides属性,会动态的为当前bean
		 * 	生成代理并使用对应的拦截器为bean做增强处理,相关逻辑实现在bean的实例化部分详细介绍.
		 * 		但是,这里要提到的是,对于方法的匹配来讲,如果一个类中存在若干个重载方法,那么在函数调用及增强的时候还需要根据参数类型进行匹配,
		 * 	来最终确认当前调用的到底是那个函数.但是,Spring将一部分匹配工作在这里完成了,如果当前类中的方法只有一个,那么就设置重载该方法没有
		 * 	被重载,这样在后续调用的时候变可以直接使用找到的方法,而不需要进行方法的参数匹配验证了,而且还可以提前对方法存在性进行验证,正可谓
		 * 	一箭双雕.
		 */
